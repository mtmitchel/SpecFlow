...
import { continueInitiativeValidation, generateInitiativePlan } from "../../../api.js";
import { ApiError } from "../../../api/http.js";
import type { Initiative, InitiativePlanningStep, InitiativeRefinementState } from "../../../types.js";
...
import { buildValidationDraftByStep, rerunValidationQuestions } from "./planning-continuation.js";
  ...
  const handleGenerateTickets = useCallback(async (): Promise<void> => {
    ...
    const status = await withBusyAction("generate-tickets", async (signal) => {
      const recoverPlanValidationFailure = async (error: unknown): Promise<boolean> => {
        ...
        generationError = (error as Error).message?.trim() || "Ticket generation failed.";
        return false;
      };
    ...
    if (status === "failed" && initiative.phases.length === 0 && initiativeTicketCount === 0) {
