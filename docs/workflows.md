...
10. Once Validation passes or is explicitly overridden, SpecFlow commits the ticket plan and moves into **Tickets**. Tickets should read as an execution board that keeps the ordered phases visible, lets the user select a phase, and shows that phase as a status-based kanban board using the existing ticket states. Clicking a ticket should open the ticket workspace directly. Tickets should not own planning blockers, review questions, or coverage dumps anymore.
