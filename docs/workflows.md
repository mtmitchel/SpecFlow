...
4. Once the intake is answered or explicitly deferred, the user generates the Brief. SpecFlow then runs a **Brief review** artifact, but the main path remains the artifact review screen rather than a separate blocking checkpoint surface.
5. User moves into **Core flows**. Before the first draft, the planner runs a required short consultation that locks the primary journey, a meaningful branch or destructive path, and a flow condition that changes the map. Later checks can add one targeted failure or degraded-path follow-up when needed. The user then generates the Core flows artifact. **Review core flows** and **Cross-check brief and core flows** remain available as secondary review artifacts instead of primary navigation gates.
6. User moves into **PRD**. Before the first draft, the planner asks at least one required scope-setting question. Later PRD checks stay focused on user-visible behavior, governing rules, v1 priorities, and non-goals before the user generates the PRD. **Review PRD** plus **Cross-check core flows and PRD** remain secondary review artifacts on the document review surface.
7. User moves into **Tech spec**. Before the first draft, the planner asks at least one required architecture question. Later checks stay focused on implementation tradeoffs such as data flow, existing-system constraints, compatibility, performance, operations, risk, and quality strategy before the user generates the Tech spec. **Review tech spec**, **Cross-check PRD and tech spec**, and the **spec-set review** remain secondary review artifacts instead of blocking the next artifact-phase handoff.
