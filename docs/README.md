...
Browser end-to-end coverage now lives in the root Playwright harness. Use `npm run test:e2e` to run the main initiative workflow and the core-flows review-back/update path against the legacy web runtime with a deterministic fake planner/verifier backend.
