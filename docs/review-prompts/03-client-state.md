...
You are reviewing the React client of a developer tool. It uses React 19, React Router v7, desktop transport through the Tauri bridge, and no client-side state management library beyond the root `ArtifactsSnapshot` state in `App.tsx`.

Review for state management correctness, race conditions, memory leaks, and cleanup issues across the desktop bridge, request-scoped notifications, and snapshot refresh flows.

## Key files to read from the repo

- `packages/client/src/App.tsx` -- root component, snapshot state
- `packages/client/src/api/transport.ts` -- desktop transport, request timing, and mutation refresh behavior
- `packages/client/src/app/hooks/use-verification-stream.ts` -- verification polling and reconnect state
...
- `useVerificationStream` is desktop-only. It polls persisted run state after reconnect or completion and must not thrash or resurrect stale work.
...
1. **Stale closures**: `useVerificationStream` still depends on a derived `runId` and an `onRefresh` callback that comes from `App.tsx`. If the ticket's `runId` changes after export or verification, does the hook tear down the previous polling loop cleanly and avoid reconnecting stale work? Is every callback in scope aligned with the current `ticketId`, `runId`, and mounted state?
...
4. **Memory leaks**: Check timer cleanup, blob URL revocation, async event listener teardown, and desktop pending-request cleanup. React 19 no longer warns on every setState-after-unmount path, so verify the cleanup explicitly.

5. **Prop drilling vs. hook coupling**: `CaptureVerifySection` still receives verification setters from `useVerificationStream`. Can the section drive `verifyState` into a value that conflicts with the hook’s own reconnect and completion flow?
