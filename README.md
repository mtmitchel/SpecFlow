...
- **Validation-owned ticket readiness**: Validation now owns the last planning gate before tickets are committed. It drafts the ticket plan, repairs weak ticket coverage when the specs already imply the missing work, reroutes true upstream ambiguities into in-place follow-up questions, and only commits tickets once the plan is clear or explicitly overridden.
