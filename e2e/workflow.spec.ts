...
const chooseSurveyOption = async (page: Parameters<typeof test>[0]["page"], optionText: string): Promise<void> => {
...
const continueSurvey = async (page: Parameters<typeof test>[0]["page"], label = "Continue"): Promise<void> => {
  const surveyCard = page.locator(".planning-survey-card-active");
  await surveyCard.getByRole("button", { name: new RegExp(`^${escapeRegex(label)}$`) }).click();
};
...
const createInitiativeThroughCoreFlowsReview = async (
  ...
  await expect(page.getByRole("heading", { name: "What primary problem should v1 solve?" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "What should feel true if v1 succeeds?" })).toBeVisible();
  await chooseSurveyOption(page, "Feels simple and focused");
  await continueSurvey(page);

  await expect(page.getByRole("heading", { name: "Which constraints matter from day one?" })).toBeVisible();
  await chooseSurveyOption(page, "Works offline or in unreliable network conditions");
  await continueSurvey(page);

  await expect(page.getByRole("button", { name: "Continue to core flows" })).toBeVisible();
  await expect(page.getByText("helps solo writers capture and edit notes locally")).toBeVisible();
  await page.getByRole("button", { name: "Continue to core flows" }).click();

  await expect(page.getByRole("heading", { name: "What should the main flow cover first?" })).toBeVisible();
  await chooseSurveyOption(page, "Create, edit, and keep notes on the local machine");
  await continueSurvey(page);

  await expect(page.getByRole("button", { name: "Continue to PRD" })).toBeVisible();
  await expect(page.getByText("Create a note.")).toBeVisible();
};

test.describe.configure({ mode: "serial" });

test("completes the main initiative workflow from Home to a passing run", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "What has to be true in v1?" })).toBeVisible();
  await chooseSurveyOption(page, "Capture and editing ship first, with search staying basic");
  await continueSurvey(page);

  await expect(page.getByRole("button", { name: "Continue to tech spec" })).toBeVisible();
  await expect(page.getByText("Users can create, edit, and reopen local notes.")).toBeVisible();
  await page.getByRole("button", { name: "Continue to tech spec" }).click();

  await expect(page.getByRole("heading", { name: "What implementation constraint matters most?" })).toBeVisible();
  await chooseSurveyOption(page, "Notes stay local and readable on disk");
  await continueSurvey(page);

  await expect(page.getByRole("button", { name: "Continue to tickets" })).toBeVisible();
  await expect(page.getByText("Keep note persistence local and readable on disk.")).toBeVisible();
  await page.getByRole("button", { name: "Continue to tickets" }).click();

  await expect(page.getByRole("button", { name: "Generate tickets" })).toBeVisible();
  await page.getByRole("button", { name: "Generate tickets" }).click();

  await expect(page.getByRole("button", { name: "Open first ticket" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Coverage check" })).toBeVisible();
  await page.getByRole("button", { name: "Open first ticket" }).click();

  await expect(page.getByRole("heading", { name: "Persist local note edits" })).toBeVisible();
  ...
  await page.locator("textarea.multiline").last().fill(
  ...
  await expect(page.getByRole("link", { name: "Back to ticket" })).toBeVisible();
});

test("reopens core flows from review and updates the artifact after an answer change", async ({ page }) => {
  await createInitiativeThroughCoreFlowsReview(
    page,
  ...
  await expect(page.getByRole("heading", { name: "What should the main flow cover first?" })).toBeVisible();
  await chooseSurveyOption(page, "Search and organize an existing note library");
  await continueSurvey(page, "Update core flows");

  await expect(page.getByRole("heading", { name: "How should the app handle notes that are created but left empty?" })).toBeVisible();
  ...
  await expect(page.getByText("the app moves it to Trash automatically")).toBeVisible();
  await expect(page.getByRole("button", { name: "Continue to PRD" })).toBeVisible();
});
